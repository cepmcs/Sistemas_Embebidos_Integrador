...
ser = serial.Serial(port='/dev/ttyACM0', baudrate=115200, bytesize=serial.EIGHTBITS,
parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=30,xonxoff=False, rtscts=False,
write_timeout=30, dsrdtr=False, inter_byte_timeout=None, exclusive=None)
...
data=""
while "Initialize MPU6050" not in data:
...
while True:
    ...
    while True:
        current_time = time.time()  # Obtener el tiempo actual en cada iteración
        if current_time - start_time > timeout:
            break  # Salir del bucle después de 10 segundos

        data = ser.readline().decode("utf-8")
        numbers_str_list = data.split("_")
        if len(numbers_str_list) != 6:
            # Convertir cada string numérico a float
            numbers_float_list = [float(number.strip()) for number in numbers_str_list]

            if len(numbers_float_list) == 6:
                jsonList.append({"Xac": numbers_float_list[0], "Yac": numbers_float_list[1], "Zac": numbers_float_list[2],
                            "Roll": numbers_float_list[3], "Pitch": numbers_float_list[4], "Yaw": numbers_float_list[5],
                            "Timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
        
        time.sleep(0.1)

    stream_data_to_bigquery("savvy-climber-426522-e8", "Integrador_Embebidos", "Sistemas_Embebidos", jsonList)
    print("enviado")